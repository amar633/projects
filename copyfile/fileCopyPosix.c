...
int main() {
      char ifpath[MAX_PATH + 1];
      /* name of input file */
      char ofpath[MAX_PATH + 1];
      /* name of output file */
      char scanf_string[10];
      /* argument string for scanf() */
      struct stat ifstat;
      /* result of stat() call */
      char * bigbuf;
      /* pointer to buffer */
      ...
      size_t hunk;
      /* number of bytes to transfer in one piece */
      size_t left;
      ...
      if (scanf(scanf_string, ifpath) != 1) {
      ...
      /* left is the amount left * to copy. Start it out * with the size of the * whole file. */
      if (hunk > HUNK_MAX)
         hunk = HUNK_MAX;
      ...
      /* Open the input file */
      if ((ifdes = open(ifpath, O_RDONLY)) == -1)  {
		  	printf("Cannot open file %s \n", ifpath); 
			exit(0); 
	  };;
      /* Now that we have the input file open, ask for the * path for the output file */
      (void) printf("Output file: ");
      if (scanf(scanf_string, ofpath) != 1)  {
		  	printf("Cannot open file %s \n", ofpath); 
			exit(0); 
	  };;
      /* Open the output file */
      if ((ofdes = open(ofpath, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR)) == -1)  {
		  	printf("Problem with destination file file %s \n", ifpath); 
			exit(0); 
	  };;
      while (left > 0) {
      ...
      (void) printf("%s copied to %s (%ld bytes)\n", ifpath, ofpath, ifstat.st_size);
      return (0);
}